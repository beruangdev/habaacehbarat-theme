...
<div class="group flex gap-3 rounded border hover:bg-background-200/30 dark:hover:bg-background-925/50 w-full cursor-pointer relative">
  ...
  <a href="<?= get_permalink() ?>" class="min-w-[20%] w-[20%]">
    <div class="aspect-[16/11] overflow-hidden w-full h-full mb-3 rounded-l">
      <img class="w-full h-full object-cover object-center group-hover:scale-110" src="<?= $image_src ?>" srcset="<?= $image_srcset ?>" alt="">
    </div>
  </a>
  <div class="flex flex-col justify-between pb-3 pt-4 pr-2">
    <a href="<?= get_permalink() ?>" class="no-underline hover:opacity-80 mb-3">
      <h4 class="leading-5 group-hover:text-primary-700 group-hover:dark:text-primary-500"><?= get_the_title(); ?></h4>
    </a>
